  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if(shpEmail){
          const response = await axios.get(`http://192.168.0.105:5000/api/shp/profile?email=${shpEmail}`);
          ...
          <div className="navbar" onClick={toggleNavMenu} onMouseEnter={toggleNavMenu} onMouseLeave={closeNavMenu}>
            ...
            {navOpen && (
              <div className="nav-menu">
                <Link to="/shopprofile"><button className="nav-item">Shop Profile</button></Link>
